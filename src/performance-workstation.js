...
export class Performance extends BasePerformance {
  constructor(onPlay) {
    ...
    this.keyMap = ['w','s','e','d','r','f','t','g','y','h','u','j','i','k','o','l'];
  }
  snapshotControls() {
  ...
  bindComputerKeys() {
    this.heldKeys=new Map();this.keyDown=async e=>{
      ...
      const index=this.keyMap.indexOf(key);if(index<0)return;e.preventDefault();const midi=(this.keyboardStart+1)*12+index;if(!isScaleNote(midi,this.rootNote,this.scale))return;const source=`key-${e.code}`;this.heldKeys.set(e.code,source);if(!this.active)await this.play();if(this.heldKeys.has(e.code))this.triggerOn(source,midi);
