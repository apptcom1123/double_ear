...
import { NOTE_NAMES, SCALES, PROGRESSIONS, DRUMS, KITS, FX, isScaleNote, scaleChord, degreeMidi } from './music-tools.js';
...
export class Performance extends BasePerformance {
  ...
  noteOn(midi, record = true) {
    ...
    const ctx=this.host.context,osc=ctx.createOscillator(),gain=ctx.createGain(),pan=ctx.createStereoPanner(),lfo=ctx.createOscillator(),vibrato=ctx.createGain();
    osc.type='triangle';osc.frequency.value=440*2**((midi-69)/12);osc.detune.value=this.pitch*200;lfo.frequency.value=5.2;vibrato.gain.value=this.modulation*45;
    gain.gain.setValueAtTime(0,ctx.currentTime);gain.gain.linearRampToValueAtTime(.065,ctx.currentTime+.015);lfo.connect(vibrato).connect(osc.detune);osc.connect(gain).connect(pan).connect(this.instrumentBus);osc.start();lfo.start();
    this.notes.set(midi,{osc,gain,pan,lfo,vibrato,released:false,slide:0,expression:0});if(record)this.capture({type:'noteOn',midi});this.paintKeys();
  }
  noteOff(midi,record=true,force=false) {
    ...
    const now=this.host.context.currentTime;note.gain.gain.cancelScheduledValues(now);note.gain.gain.setTargetAtTime(0,now,.035);note.osc.stop(now+.2);note.lfo.stop(now+.2);note.osc.onended=()=>[note.osc,note.gain,note.pan,note.lfo,note.vibrato].forEach(node=>node.disconnect());this.notes.delete(midi);this.paintKeys();
  }
  ...
  async drumHit(pad,velocity=this.drumVelocity,record=true) {
    if(!this.active){await this.play();if(!this.active)return;}const ctx=this.host.context,index=Number(pad),kit=this.drumKit,level=clamp(velocity)*.18,now=ctx.currentTime,destination=this.instrumentBus,tonal=[0,5,6,8,12,13,14,15].includes(index);
    if(tonal){const osc=ctx.createOscillator(),gain=ctx.createGain(),base=[52,180,130,520,340,220,760,410][[0,5,6,8,12,13,14,15].indexOf(index)];osc.type=kit==='808'?'sine':kit==='lofi'?'triangle':'square';osc.frequency.setValueAtTime(base*(kit==='pop'?1.15:1),now);osc.frequency.exponentialRampToValueAtTime(Math.max(32,base*.55),now+.16);gain.gain.setValueAtTime(level,now);gain.gain.exponentialRampToValueAtTime(.0001,now+.12+(index===0?.35:.08));osc.connect(gain).connect(destination);osc.start();osc.stop(now+.5);osc.onended=()=>{osc.disconnect();gain.disconnect();};}
    else{const length=Math.ceil(ctx.sampleRate*(index===3||index===10?.65:.18)),buffer=ctx.createBuffer(1,length,ctx.sampleRate),data=buffer.getChannelData(0);for(let i=0;i<length;i++)data[i]=(Math.random()*2-1)*Math.exp(-i/(length*(kit==='lofi'?.22:.12)));const source=ctx.createBufferSource(),filter=ctx.createBiquadFilter(),gain=ctx.createGain();source.buffer=buffer;filter.type=index===1||index===4?'bandpass':'highpass';filter.frequency.value=kit==='lofi'?1400:3500+index*170;gain.gain.value=level;source.connect(filter).connect(gain).connect(destination);source.start();source.onended=()=>{source.disconnect();filter.disconnect();gain.disconnect();};}
    if(record)this.capture({type:'drum',pad:index,kit,velocity:clamp(velocity)});this.flashPad(index);
  }
  ...
  applyEvent(event) {
    ...
    if(event.type==='drum'){this.drumKit=event.kit;this.drumHit(event.pad,event.velocity,false);}
  ...
  render(root) {
    ...
    <div class="panel drum-panel"><div class="tool-title"><strong>1 × 4 打擊墊</strong><select id="drum-kit">${Object.entries(KITS).map(([id,name])=>`<option value="${id}">${name}</option>`).join('')}</select></div><div class="drum-toolbar"><span id="drum-banks"></span><label>力度 <input id="drum-velocity" type="range" min=".2" max="1" step=".05" value="${this.drumVelocity}"></label></div><div id="drum-pads" class="drum-pads"></div></div></section>
    <section class="performance-tools"><div class="panel fx-panel"><div class="tool-title"><strong>Built-in FX</strong><select id="fx-select">${Object.entries(FX).map(([id,name])=>`<option value="${id}">${name}</option>`).join('')}</select></div><label class="control"><span class="control-head">效果量<output id="fx-value"></output></span><input id="fx-amount" type="range" min="0" max="1" step=".01" value="${this.fxAmount}"></label></div><div class="panel matrix-panel"><div class="tool-title"><strong>控制矩陣</strong><small>自訂 CC + XY Automation</small></div><div id="cc-controls" class="cc-controls"></div><div class="xy-wrap"><div id="xy-pad" class="xy-pad" aria-label="XY 觸控板"><i></i></div><label>X CC <input id="xy-cc-x" type="number" min="0" max="127" value="${this.xy.ccX}"></label><label>Y CC <input id="xy-cc-y" type="number" min="0" max="127" value="${this.xy.ccY}"></label></div></div></section>
  ...
  renderKeyboard() {
    ...
    for(let semitone=0;semitone<=24;semitone++){
      const midi=start+semitone,black=[1,3,6,8,10].includes(semitone%12),button=document.createElement('button');button.type='button';button.dataset.midi=midi;button.className=`piano-key ${black?'black':'white'}`;button.textContent=NOTE_NAMES[midi%12];button.setAttribute('aria-label',`${NOTE_NAMES[midi%12]}${Math.floor(midi/12)-1}`);button.hidden=!isScaleNote(midi,this.rootNote,this.scale);
  ...
  bindDrums() {
    const root=this.root;root.querySelector('#drum-kit').value=this.drumKit;root.querySelector('#drum-kit').onchange=e=>{this.drumKit=e.target.value;this.renderDrums();};root.querySelector('#drum-velocity').oninput=e=>{this.drumVelocity=Number(e.target.value);};
    const banks=root.querySelector('#drum-banks');for(let i=0;i<4;i++){const button=document.createElement('button');button.type='button';button.textContent=i+1;button.onclick=()=>{this.drumBank=i;this.renderDrums();};banks.append(button);}this.renderDrums();
  }
  renderDrums() {
    const host=this.root.querySelector('#drum-pads');if(!host)return;host.replaceChildren();
    for(let i=0;i<4;i++){const index=this.drumBank*4+i,button=document.createElement('button');button.type='button';button.dataset.pad=index;button.innerHTML=`<small>${String(index+1).padStart(2,'0')}</small><strong>${DRUMS[index]}</strong>`;button.onpointerdown=e=>{const rect=button.getBoundingClientRect(),velocity=clamp(1-(e.clientY-rect.top)/rect.height*.7,.25,1);this.drumHit(index,velocity);};host.append(button);}
  ...
  bindComputerKeys() {
    this.heldKeys=new Map();this.keyDown=async e=>{if(/INPUT|TEXTAREA|SELECT/.test(e.target.tagName)||e.target.isContentEditable||this.root.querySelector('dialog').open||e.repeat)return;const index=this.keyMap.indexOf(e.key.toLowerCase());if(index<0)return;e.preventDefault();const midi=(this.keyboardStart+1)*12+index;if(!isScaleNote(midi,this.rootNote,this.scale))return;const source=`key-${e.code}`;this.heldKeys.set(e.code,source);if(!this.active)await this.play();if(this.heldKeys.has(e.code))this.triggerOn(source,midi);};this.keyUp=e=>{const source=this.heldKeys.get(e.code);if(source)this.triggerOff(source);this.heldKeys.delete(e.code);};this.blur=()=>{this.stopArp();this.releaseNotes();this.heldKeys.clear();};window.addEventListener('keydown',this.keyDown);window.addEventListener('keyup',this.keyUp);window.addEventListener('blur',this.blur);
  }
  ...
  paintAll(){this.paintSlots();this.renderKeyboard();this.renderDrums();this.paintExpression();this.paintFX();this.paintXY();this.paintTakes();this.paint();}
}
